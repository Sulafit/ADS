...
class Deque{
    ...
    void erase(int n,int &temp){
        ...
        // cout<<need->val<<" ";
        temp = need -> val;
        if(need -> prev && need -> next!= nullptr){ // tail
            need -> prev -> next = need -> next;
            need -> next -> prev = need -> prev;
            sz--;
        }
        else if(need -> prev == nullptr && need -> next == nullptr){ // onely one
            head = tail = nullptr;
            sz--;
        }
    ...
    void cycleleft(int k){
        int cnt=0;
        Node* cur = head;
        while(cur){
            if(cnt == k%size()){head = cur;return;}
            else{
